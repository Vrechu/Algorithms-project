...
class SampleDungeon : Dungeon
{
	...
	protected override void generate(int pMinimumRoomSize)
	{
		...
		closedRooms.Add(new Room(0, 0, size.Width/2+1, size.Height, pMinimumRoomSize));
		//right room from half of screen to the end
		closedRooms.Add(new Room(size.Width/2, 0, size.Width/2, size.Height, pMinimumRoomSize));
